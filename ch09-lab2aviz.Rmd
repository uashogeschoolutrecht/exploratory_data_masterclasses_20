...
## Conclusions: draw conclusions about growth rate (slope paramter is proxy for growth rate)
...
```{r, inspect_airq, options_exercises}
head(airq)
...
```{r, adapt_names_airq, options_exercises}
...
```{r, all_data_airq, options_exercises}
names(airq)
plot_1c <-  airq %>%
...
```{r, overplotting_airq, options_exercises}
names(airq)
plot_1d <-  airq %>%
  ggplot(aes(x = month, y = wind)) +
...
```{r, line_wind_month, options_exercises}
names(airq)
plot_1e <-  airq %>%
  ggplot(aes(x = month, y = wind)) +
  geom_point(aes(), alpha = 0.4, position = "jitter")  +
...
```{r, high_burden, options_exercises}
tb_burden %>%
...
```{r, incidence_tb, options_exercises}
tb_data_selected %>%
  as_tibble() %>%
...
```{r, highest_tb_incidence, options_exercises}
tb_data_selected %>%
  as_tibble() %>%
  mutate(incidence_1e3 = (e_inc_num/e_pop_num)*1000) %>%
  dplyr::filter(incidence_1e3 > 5,
...
```{r, average_inc_tb, options_exercises}
...
```{r, 5a, options_exercises, eval=FALSE}
joined_data %>%
...
```{r, 5b, options_exercises, eval=FALSE}
joined_data %>%
